...
<\/script>
`,le=()=>{console.log("I'm working");let e=document.querySelector(".typeJsText");console.log("typeJsText",e);let s=["FULLY.","SUCESS.","AUTOMATED.","VALVE PURCHASING.","PROCCESSES."],t=0,i=0,a=!1;function o(){e?a?i>=0?(e.innerHTML=e.innerHTML.substring(0,i),i--,setTimeout(o,100)):(a=!1,t=(t+1)%s.length,setTimeout(o,500)):i<s[t].length?(e.innerHTML+=s[t].charAt(i),i++,setTimeout(o,100)):setTimeout(()=>{a=!0,o()},1e3):console.log("Element .typeJsText not found")}o()},re=()=>{const e=document.querySelectorAll(".dropdown");if(console.log("Number of dropdowns found:",e.length),e.length===0){console.warn("No dropdown elements found!");return}e.forEach(s=>{const t=s,i=t.querySelector(".dropdown-content");if(!i){console.warn("No dropdown-content element found in a dropdown!");return}let a;const o=()=>{console.log("Showing dropdown content for",t),i.style.display="block",a&&window.clearTimeout(a)},n=()=>{a=window.setTimeout(()=>{console.log("Hiding dropdown content for",t),i.style.display="none"},1500)};t.addEventListener("mouseover",o),i.addEventListener("mouseover",o),t.addEventListener("mouseleave",n),i.addEventListener("mouseleave",n)})},R=()=>{window.addEventListener("scroll",()=>{const e=document.querySelector(".parallax-background");if(e){const s=window.pageYOffset;e.style.backgroundPositionY=`${s*.2}px`}})},ce=()=>{const e=window.scrollY||document.documentElement.scrollTop;console.log("Scroll Position:",e,"pixels");const s=document.querySelector(".sticky-header-container");e>75?window.hasOwnProperty("processActivated")||(console.log("do it here"),s&&!s.classList.contains("animate-sticky-header")&&(s.classList.add("animate-sticky-header"),console.log("Animation triggered"),setTimeout(()=>{},3e3))):s.classList.remove("animate-sticky-header")},de=`
...
`,u=document.getElementById("app"),pe=()=>{h("/",()=>{u.innerHTML=te,le(),re(),window.addEventListener("scroll",ce)}),h("/about-us",()=>{u.innerHTML=ae}),h("/steps",()=>{u.innerHTML=ie,R()}),h("/gate-valves",()=>{u.innerHTML=oe}),h("/globe-valves",()=>{u.innerHTML=de}),h("/certifications",()=>{u.innerHTML=ne,R()}),h()};document.addEventListener("DOMContentLoaded",()=>{pe()});
